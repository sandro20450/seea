...
def carregar_usuarios():
    try:
        ...
        if gc:
            ...
            usuarios = {}
            for r in records:
                usuarios[str(r['usuario'])] = { "senha": str(r['senha']), "perfil": str(r['perfil']).lower(), "nome": str(r['nome']) }
            return usuarios
    except Exception as e:
        st.sidebar.error("⚠️ Banco Base_SEEA não encontrado. Usando modo de emergência.")
    # Fallback/Emergência caso a planilha não exista ainda
    return {"admin": {"senha": "admin", "perfil": "diretoria", "nome": "Diretoria MOCK"}}
...
def carregar_alunos(turma):
    try:
        ...
        return ["Aluno Teste 1", "Aluno Teste 2"] # Fallback
...
if st.session_state.usuario_logado is None:
    ...
    with aba_freq:
        ...
        with col_turma: selecao_turma = st.selectbox("Turma:", ["6º Ano A", "7º Ano B", "8º Ano A"], key="freq_turma")
        ...
        # BUSCANDO ALUNOS REAIS DO GOOGLE SHEETS
    ...
    with aba_notas:
        if not st.session_state.diario_aberto:
            ...
            c1, c2 = st.columns(2)
            with c1: st.button("+ Nova Atividade", use_container_width=True)
            with c2: st.button("💾 Salvar Diário de Notas", type="primary", use_container_width=True)
    ...
    with aba_ia:
        ...
        if not ia_configurada:
            st.error("⚠️ **Sistema Desconectado:** A chave da API do Gemini não foi encontrada no painel Secrets. Adicione a variável `GEMINI_API_KEY` para ativar a IA.")
        else:
            ...
            if gerar_prova_btn and assunto:
                with st.spinner("Conectando ao núcleo de IA do Gemini... Elaborando prova inédita..."):
                    try:
                        ...
                        prompt = f"""
                        Você é um professor experiente elaborando uma prova escolar.
                        Assunto: {assunto}
                        Nível de Dificuldade: {nivel_dif}
                        Quantidade de Questões: {qtd_quest}
                        Tipo de Questões: {tipo_quest}
                        Peso de cada questão: {peso_quest} pontos.
                        
                        Por favor, gere uma avaliação completa e formatada. 
                        Inclua um cabeçalho escolar no topo (Escola Projeto Saber, Nome, Data).
                        As questões devem ser desafiadoras e adequadas ao nível solicitado.
                        NÃO coloque o gabarito junto com a prova. 
                        Obrigatório: Gere o GABARITO COMPLETO apenas no final do documento, após um divisor de linha, claramente marcado como "GABARITO DO PROFESSOR".
                        """
